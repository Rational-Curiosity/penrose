    ...
    { $struct:ident } => {
        impl $crate::core::xconnection::XState for $struct {
                ...
                let (x, y, id) = match win_id {
                    ...
                    None => {
                        ...
                        ((x + w - config.gap_px() - config.border_px() - 1),
                         (y + h - config.gap_px() - config.border_px() - 1), self.api.root())
                    }
                };
            ...
            fn cursor_over_client(&self, id: Xid) -> bool {
                if let Ok(cursor_position) = self.api.cursor_position()
                {
                    if let Ok(client_geometry) = self.api.client_geometry(id)
                    {
